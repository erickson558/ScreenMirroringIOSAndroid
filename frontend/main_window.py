...
class MainWindow:
    ...
    def __init__(
        ...
        self._window_state = "embedded"  # embedded, detached, or fullscreen
    ...
    def _build_ui(self, announce_init: bool = True) -> None:
        ...
        self._btn_window_state = ttk.Button(
            top,
            text=self._tr("btn_detach_window"),
            underline=0,
            style="Glass.TButton",
            command=self._toggle_window_state,
            state="disabled",
        )
        self._btn_window_state.pack(side="left", padx=(8, 0))
    ...
    def _toggle_window_state(self) -> None:
        """Toggle between embedded and detached window states."""
        if self._window_state == "embedded":
            self._detach_window()
        elif self._window_state == "detached":
            self._attach_window()

    def _update_window_state_button(self) -> None:
        """Update button text based on current window state."""
        if self._window_state == "embedded" and self._embedded_window_hwnd is not None:
            self._btn_window_state.configure(text=self._tr("btn_detach_window"))
            self._btn_window_state.configure(state="normal")
        elif self._window_state == "detached" and self._detached_window_hwnd is not None:
            self._btn_window_state.configure(text=self._tr("btn_attach_window"))
            self._btn_window_state.configure(state="normal")
        else:
            self._btn_window_state.configure(state="disabled")

    def _detach_window(self) -> None:
        """Detach the embedded window and show it as an independent floating window."""
        if os.name != "nt":
            return
        ...
        try:
            ...
            # Position to the right of main window, centered vertically
            try:
                main_x = self._root.winfo_rootx()
                main_y = self._root.winfo_rooty()
                main_width = self._root.winfo_width()
            ...
            # Keep window size within screen bounds
            if x + window_width > screen_width:
                x = max(0, screen_width - window_width - 20)
            
            SWP_NOZORDER = 0x0004
            user32.SetWindowPos(hwnd, 0, x, y, window_width, window_height, SWP_NOZORDER)
    ...
    def _attach_window(self) -> None:
        """Re-attach the floating window back to the preview panel."""
        if os.name != "nt":
            return
        
        hwnd = self._detached_window_hwnd
        if hwnd is None:
            return
        ...
        try:
            ...
            # Reposition to embed back in preview panel
            x = self._preview_host_frame.winfo_rootx()
            y = self._preview_host_frame.winfo_rooty()
            width = max(self._preview_host_frame.winfo_width(), 300)
            height = max(self._preview_host_frame.winfo_height(), 540)
            ...
            self._append_log("[PISTA] Ventana de iPhone acoplada de nuevo al panel de preview.")
            self._update_window_state_button()
        except Exception as e:
            self._append_log(f"[ERROR] No se pudo acoplar la ventana: {e}")
    ...
    def _release_embedded_window(self) -> None:
        ...
        hwnd = self._embedded_window_hwnd or self._detached_window_hwnd
        self._embedded_window_hwnd = None
        self._detached_window_hwnd = None
        self._window_state = "embedded"
